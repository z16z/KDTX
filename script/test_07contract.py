...
yaml_path = os.path.join(parent_dir, "data", "contract.yaml")
...
class TestContract:
    ...
    @pytest.fixture(scope='class')
    def contract_api(self,global_test_data):
        """初始化登录接口"""
    ...
    def test_contract(self,contract_api,case):
        """执行登录流程：先获取验证码，再登录"""
        print(f'执行用例：{case['name']}')
        response = contract_api.send_reqeust(case)
